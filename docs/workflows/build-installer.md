...
- MediaLens prepares a managed Python bootstrap before creating model venvs. If `tools\python\python.3.12.10.nupkg` exists at build time, `scripts\build_installer.ps1` copies it into the app bundle; otherwise the installed app downloads and verifies that official NuGet Python package on first model install.
- Do not commit `tools\python\python.3.12.10.nupkg`; keep it as an optional local build input when you want a fully bundled Python bootstrap.
