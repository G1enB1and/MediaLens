...
- The installer does not prompt for or download optional AI runtimes during setup. MediaLens shows Local AI Models setup after install/update and also exposes model installation from Settings > AI.
- MediaLens prepares a managed Python bootstrap before creating model venvs. If `tools\python\python-3.12.10-amd64.exe` exists at build time, `scripts\build_installer.ps1` copies it into the app bundle; otherwise the installed app downloads and verifies that official Python.org installer on first model install.
- Do not commit `tools\python\python-3.12.10-amd64.exe`; keep it as an optional local build input when you want a fully bundled Python bootstrap.
